...
export function DashboardTrustCallout() {
  return (
    <aside
      className="rounded-2xl border border-[color-mix(in_srgb,var(--amara-smoked-teal)_22%,transparent)] bg-gradient-to-br from-[color-mix(in_srgb,var(--amara-smoked-teal)_10%,white)] to-white/80 p-5 shadow-[0_8px_24px_-12px_rgba(67,105,106,0.18)]"
    ...
    >
      <div className="flex items-start gap-3">
        <span
          ...
          aria-hidden
        >
          <AmaraIcon name="shield-check" className="h-4 w-4 text-smoked-teal" />
        </span>
        <div>
      ...
      <ul className="mt-4 space-y-2">
        {DASHBOARD_TRUST_FEATURES.map((feature) => (
          <li
            key={feature}
            className="flex gap-2 text-xs leading-relaxed text-[color-mix(in_srgb,var(--amara-espresso)_75%,white)] sm:text-sm"
          >
            <AmaraIcon
              name="lock-secure"
