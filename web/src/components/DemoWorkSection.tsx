...
function DemoShowcase() {
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center justify-between gap-3 sm:justify-end">
          <p className="text-xs font-semibold uppercase tracking-wider text-smoked-teal">
            ...
            <span className="text-[color-mix(in_srgb,var(--amila-espresso)_45%,white)]">
              {" "}
              / {total}
            </span>
          </p>
          <div className="flex gap-2">
            <button
              ...
              className="flex h-10 w-10 items-center justify-center rounded-full border border-[color-mix(in_srgb,var(--amila-nude-stone)_45%,transparent)] bg-white/80 text-espresso transition-colors hover:border-smoked-teal hover:text-smoked-teal"
            >
          ...
          <div className="flex flex-col p-6 sm:p-7 lg:p-8">
            ...
            <div className="mt-6 flex flex-1 flex-col gap-5 text-sm">
              <div>
                ...
                <p className="mt-2 text-base leading-relaxed text-[color-mix(in_srgb,var(--amila-espresso)_78%,white)]">
                  {demo.problemSolved}
                </p>
              </div>
              <div>
                ...
                <ul className="mt-3 flex flex-wrap gap-2">
                  {demo.keyMetrics.map((m) => (
                    <li
                      key={m}
                      className="rounded-full bg-gradient-to-r from-[color-mix(in_srgb,var(--amila-nude-stone)_28%,white)] to-white px-3.5 py-1.5 text-xs font-semibold text-espresso ring-1 ring-[color-mix(in_srgb,var(--amila-nude-stone)_38%,transparent)]"
                    >
                      {m}
                    </li>
                  ))}
              ...
              <div>
                ...
                <ul className="mt-3 space-y-2 text-[color-mix(in_srgb,var(--amila-espresso)_78%,white)]">
              ...
              {DEMOS.map((_, i) => (
                <button
                  ...
                  className={`h-1.5 rounded-full transition-all ${
                    i === activeIndex
                      ? "bg-smoked-teal"
                      : "bg-[color-mix(in_srgb,var(--amila-nude-stone)_50%,white)] hover:bg-smoked-teal/50"
                  }`}
                />
              ))}
            </div>
      ...
      <p className="mt-4 text-center text-[11px] text-[color-mix(in_srgb,var(--amila-espresso)_55%,white)] sm:text-xs">
    ...
    <SectionBackdropWide id="demo-work" tone="warm" motif>
      <Reveal>
        <SectionHeading
          ...
          subtitle="Examples of the insights Amila can deliver. From retention risk and inventory waste, to profitability and executive decision support"
        />
      </Reveal>
